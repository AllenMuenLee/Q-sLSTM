# 2025 06 20: Need to fix: the concat seems to be not correct? (all zeros and not updated?????)
...
class VQC(nn.Module):
	...
	def forward(self, X):
		...
		y_preds = torch.stack(self.VQC(X, self.weights, self.n_class)).T
...
class CustomQLSTMCell(nn.Module):
	...
	def forward(self, x, hidden):
		...
		# print("g_t: {}".format(g_t))
		# print("o_t.shape: {}".format(o_t.shape))
		# print("o_t: {}".format(o_t))
